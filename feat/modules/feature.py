...
#matrix = feature.greycomatrix(inds, [1], [0, np.pi/4, np.pi/2, 3*np.pi/4], levels=max_value, symmetric=False, normed=True)

ContrastStats = feature.greycoprops(matrix, 'contrast')
CorrelationtStats = feature.greycoprops(matrix, 'correlation')
...
print('Homogenity = ',np.mean(HomogenityStats))
print('Energy = ',np.mean(EnergyStats))
print('ASM = ',np.mean(ASMStats))
